...
watch( props , (val)=>{
  imagePreview.value = val.modelValue;
})
const removeImage = () => {
  console.log('aaa',imagePreview.value)
  imagePreview.value = "";
...
<template>
  <div class="mb-2">
    <label class="labelStyle" v-if="props.required">{{ translate(props.title) }}*</label>
    <label class="labelStyle" v-else>{{ translate(props.title) }}</label>
    <div
      v-if="!imagePreview"
